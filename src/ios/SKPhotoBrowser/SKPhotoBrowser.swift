...
open class SKPhotoBrowser: UIViewController {
    ...
    override open func viewDidLoad() {
        ...
        //显示文字统计就不显示pageControl
        if(!SKPhotoBrowserOptions.displayCounterLabel){
            pageControl.currentPage = currentPageIndex
            pageControl.numberOfPages = numberOfPhotos
            pageControl.frame = CGRect(x:0,y:SKMesurement.screenHeight-40,width:100,height:40)
            pageControl.center = CGPoint(x:SKMesurement.screenWidth/2,y:SKMesurement.screenHeight-40)
    ...
    private func autoFrameforPageControl (){
        //如果不显示就不执行了
        if(SKPhotoBrowserOptions.displayCounterLabel){
            return
        }
    ...
    override open func viewWillLayoutSubviews() {
        ...
        //当旋转屏幕的时候，自动计算pageControl的位置
        autoFrameforPageControl()
        // where did start
